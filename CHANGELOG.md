...
## [V9.14.20 — لوحة الراحة على اليسار كبطاقة مدمجة] — 2026-06-03

نقل لوحة مؤقت الراحة من الوسط إلى **أسفل اليسار** كبطاقة واحدة مدمجة تضم كل مشتملاتها ([`css/styles.css`](css/styles.css)): العدّاد · ابدأ/إيقاف · الإعدادات المسبقة (٤٥/٦٠/٩٠/١٢٠) · +٣٠/تخطي · بطاقة «التمرين التالي» + عرض البديل. خلفية وحدّ وظلّ موحّد (radius ١٨)، وتُرفع تلقائياً **فوق الشريط السفلي وشريط الجلسة** على الموبايل (لا تداخل).

تحقّق آلي بـ headless Chrome (جلسة فعلية): اللوحة على اليسار (left ١٢px · عرض ٢٢٠px)، فوق الشريط السفلي بلا تداخل، بكل مشتملاتها (التمرين التالي + ٦ أزرار)، صفر أخطاء. bump إلى 9.14.20.

---

## [V9.14.19 — تدفّق Plan B كامل: بدائل بنسب تطابق + كل الأجهزة + نطاق تطبيق] — 2026-06-03

تحسين تدفّق «الجهاز مشغول؟» ([`js/substitutions.js`](js/substitutions.js) + [`css/styles.css`](css/styles.css)) — زر واحد ثم كل شيء في النافذة السفلية:
- البدائل تظهر مع **نسب تطابق العضلة** (شريط + ٪) ومرتّبة (peak-friendly وقت الذروة).
- لكل بديل **نطاق تطبيق**: هذا السيت · كل اليوم · دائماً (كل الأيام).
- زر دائم **«لم يناسبك شيء؟ اختر من كل الأجهزة المتاحة»** (كان يظهر فقط في الحالة الفارغة).
- لوحة كل الأجهزة الآن فيها **محدّد نطاق** (سيت/يوم/دائماً) + **تمييز الأجهزة المتاحة في جيمك** (✓ خضراء، تُرتّب أولاً) + التطبيق بالنطاق المختار عبر `applyScopedSubstitution`.

تحقّق آلي بـ headless Chrome: النافذة تفتح بـ٤ بدائل ونِسب تطابق وأزرار نطاق، زر «كل الأجهزة» يعرض ٢٣ جهازاً مع تمييز المتاح ومحدّد نطاق، والتطبيق ينجح ويُغلق النافذة. صفر أخطاء. bump إلى 9.14.19.

---

## [V9.14.18 — تصغير زر «الجهاز مشغول؟» لشريحة مدمجة] — 2026-06-03

زر «الجهاز مشغول؟» كان صفاً كبيراً يتمدّد عمودياً (السبب: مُلحَق بـ `.step` وهي
`display:flex;align-items:stretch` فيمتدّ لكامل ارتفاع الستيب). الإصلاح
([`js/substitutions.js`](js/substitutions.js) + [`css/styles.css`](css/styles.css)):
- إلحاقه بـ `.step-body` (block) بدل `.step` → لا يتمدّد.
- شريحة صغيرة: أيقونة `repeat` الموحّدة + «مشغول؟» · padding ٤×٩ · خط ١٠٫٥px ·
  لون معلوماتي أزرق. الحجم صار ~٧٨×٣٢px بدل صف كامل. تبقى تفتح الـ bottom sheet.

تحقّق آلي: الشريحة ٧٨×٣٢px وتفتح نافذة البدائل، صفر أخطاء. bump إلى 9.14.18.

---

## [V9.14.17 — تقليل النصوص أثناء الجلسة + حالة اكتمال + ثقة أوفلاين] — 2026-06-03

5. **تقليل النصوص أثناء الجلسة** ([`js/ui-v99.js`](js/ui-v99.js) + [`css/styles.css`](css/styles.css)): يُفعَّل Focus Mode تلقائياً عند أي جلسة نشطة فيخفي الشرح العلمي · النصائح · الجداول · معلومات البرنامج · تبويبات الأيام، ويُبقي فقط تمرين اليوم النشط (الاسم · الوزن · التكرار · الراحة · البديل · حفظ · ملاحظة قصيرة).
6. **حالة اكتمال واضحة** ([`js/session.js`](js/session.js) + CSS): ✓ أخضر على رقم كل سيت محفوظ · البطاقة تتحوّل خضراء · عند اكتمال كل سيتات اليوم: شريط الجلسة السفلي يصير أخضر + toast «🎉 تم إكمال تمرين اليوم!» (مرة واحدة).
7. **عرض «آخر مرة / المقترح» أوضح**: تكبير وتوضيح نص آخر جلسة (`آخر جلسة: 15كجم × 10`) واقتراح اليوم (بلون CTA).
8. **شارة ثقة أوفلاين** ([`js/dashboard.js`](js/dashboard.js)): شارة في شريط الحالة «✓ بياناتك على جهازك · بدون إنترنت».

تحقّق آلي بـ headless Chrome (جلسة فعلية): focus يُفعَّل تلقائياً وكل الشرح/الجداول/التبويبات تختفي ويبقى تمرين اليوم، ✓ الاكتمال يظهر، شارة الأوفلاين موجودة، صفر أخطاء. bump إلى 9.14.17.

---

## [V9.14.16 — Workout Mode للموبايل: steppers + شريط جلسة سفلي] — 2026-06-03

تحسينات تنفيذ التمرين داخل الجيم:

1. **أزرار +/- للوزن والتكرار** ([`js/session.js`](js/session.js) + [`css/styles.css`](css/styles.css)): كل سيت صار له `[−] [15kg] [+]` و`[−] [10] [+]` (الوزن ±٢.٥ · التكرار ±١) عبر `stepSet` — ضبط بإصبع واحد دون لوحة مفاتيح. أزرار ٣٦px.
2. **التمرين التالي بعد الراحة**: موجود من 9.14.11 — لوحة الراحة تعرض «التالي: التمرين · الوزن · التكرارات» + +٣٠ث/تخطّي/عرض البديل. (تأكيد).
3. **Plan B كنافذة سفلية**: موجود — زر «الجهاز مشغول؟» يفتح bottom sheet بالبدائل واختيار البديل. (تأكيد).
4. **شريط جلسة سفلي ثابت** ([`index.html`](index.html) + [`js/session.js`](js/session.js)): أثناء الجلسة يظهر شريط أسفل الشاشة «{اليوم} · {مكتمل}/{إجمالي} سيت · {الوقت}» + زر «إنهاء» أحمر، في متناول الإبهام، يُحدَّث لحظياً (ticker + updateSessionProgress). فوق التنقّل السفلي على الموبايل، مع إزاحة المحتوى.

تحقّق آلي بـ headless Chrome (جلسة فعلية): الـ steppers تضبط الوزن (٠→٥)، الشريط السفلي يظهر بالبيانات، التالي بعد الراحة + Plan B bottom sheet يعملان، صفر أخطاء. bump إلى 9.14.16.

---

## [V9.14.15 — أيقونات outline موحّدة + مسافات + صفوف بطاقة التمرين] — 2026-06-03

استكمال الـ Style Guide:

- **أيقونات outline موحّدة**: مجموعة SVG sprite ([`index.html`](index.html)) ترث `currentColor` (dumbbell · timer · repeat · trending-up · utensils · calendar · target · check · alert · home · zap · award · calculator · play · x · more …). طُبّقت على الواجهة الأساسية — الشريط السفلي (٥ أيقونات) · عناوين بطاقات الرئيسية · ملخص البرنامج · إحصاءات بطاقة اليوم · الأدوات السريعة · أزرار البدء/«الجهاز مشغول؟»/«عرض الأسبوع» — بدل كثرة الإيموجي. الإيموجي يبقى في الرسائل التحفيزية والمحتوى (🎉/🧘/نصائح).
- **مسافات موحّدة**: بين البطاقات ١٤px · padding داخلي ١٦–٢٤px · فاصل قسم ٢٦px قبل التغذية والدليل.
- **صفوف واضحة داخل بطاقة الأوزان** ([`js/dashboard.js`](js/dashboard.js)): لكل تمرين وسوم واضحة — `8-10 تكرار` · `راحة ٦٠ث` · `Plan B متاح` (بأيقونات).

تحقّق آلي بـ headless Chrome (داكن): ٤٥ أيقونة موحّدة بالرئيسية، كل عناوين البطاقات بأيقونات، وسوم بطاقة التمرين تعمل، تباعد ١٤px، صفر أخطاء. bump إلى 9.14.15.

---

## [V9.14.14 — تطبيق Style Guide: لوحة ألوان متوازنة · خطوط · هرم بصري] — 2026-06-03

تحديث نظام التصميم على مستوى المتغيّرات الجذرية ([`css/styles.css`](css/styles.css)) فيسري على كل التطبيق:

- **الألوان** (`:root` الداكن): خلفية `#06080D` · سطح بطاقة `#10141D` · سطح أعلى `#171D29` · نص أساسي `#F4F6FA` · ثانوي `#9CA4B5` · ذهبي `#D4A853` · **CTA قوي `#F59E0B`** (جديد) · نجاح `#22C55E` · تنبيه `#F97316` · خطر `#EF4444` · معلومة/راحة `#38BDF8`. (الذهبي لم يعد في كل شيء — توازن أكثر.)
- **الخطوط** ([`index.html`](index.html) + [`service-worker.js`](service-worker.js)): أُضيف **IBM Plex Sans Arabic** للعناوين والنصوص الطويلة، Rubik للواجهة/الأزرار، JetBrains Mono للأرقام (الوزن/الوقت/التكرارات).
- **البطاقات**: نصف قطر ١٨px · Padding ١٦px موبايل / ٢٢-٢٤px ديسكتوب · هرم بصري (بطاقة تمرين اليوم الأقوى ظلاً وحدّاً · الأدوات السريعة أخف).
- **الأزرار**: ارتفاع ≥ ٤٨px · حواف ١٤px · أهم أزرار البدء بتدرّج ذهبي→برتقالي (CTA) بنص أسود · الخطر أحمر فقط (إنهاء/إلغاء/توقف).

تحقّق آلي بـ headless Chrome (وضع داكن): `--bg #06080D`، نص `#F4F6FA`، `--cta #F59E0B`، نصف قطر ١٨px، خط العناوين IBM Plex، أزرار CTA ٤٨px بتدرّج ذهبي→برتقالي، صفر أخطاء. حُدّثت خلفية الـ manifest. bump إلى 9.14.14.

---

## [V9.14.13 — ملخص البرنامج · أدوات سريعة · تغذية بوجبات · تقدّم مختصر · دليل عميق] — 2026-06-03

استكمال إعادة تصميم الرئيسية ([`js/dashboard.js`](js/dashboard.js) + [`css/styles.css`](css/styles.css)):

6. **ملخص البرنامج** = ٤ بطاقات بارزة فقط: ٦ أيام تمرين · ٥٥ دقيقة · ٦٠ ث راحة · ١٧٥g بروتين (شبكة ٤ أعمدة بحجم أكبر).
7. **أدوات الجيم السريعة** (`_quickTools`): بطاقة أزرار واضحة — مؤقت الراحة · حاسبة البليتات · الجهاز مشغول؟ · آخر أوزاني · (إنهاء الجلسة يظهر أثناء الجلسة فقط).
8. **تغذية اليوم**: بروتين (X/175g) + الماء (X/8 أكواب) + حالة **وجبة قبل/بعد التمرين** (✓ مكتملة / غير مكتملة من شيكات السجل اليومي) + زر «➕ سجّل وجبة».
9. **التقدّم المختصر**: ٤ أرقام فقط — جلسات هذا الأسبوع · آخر PR · إجمالي السيتات · وزن الجسم (آخر قياس) + زر واحد «افتح تقدمي» (حُذف شريط الالتزام والأزرار الثلاثة).
10. **الدليل العميق**: القسم المطوي «اعرف البرنامج بعمق» يسرد المواضيع (نظام الأزواج · لماذا ٦٠ث راحة · استراتيجية الزحمة · Plan B · الأجهزة · النصائح والتحذيرات) + رابطا الدليل/النصائح.

أُزيل `_quickStats3` المكرر مع بطاقة التقدّم. تحقّق آلي بـ headless Chrome: كل البطاقات تعرض القيم الصحيحة، حالة الوجبات من `mealsArr`، صفر أخطاء. bump إلى 9.14.13.

---

## [V9.14.12 — إعادة تصميم الرئيسية كمساعد تمرين] — 2026-06-03

تخطيط جديد للرئيسية بترتيب: **شريط حالة → Hero مختصر → بطاقة اليوم → جدول الأسبوع → «التالي عليك» → الدعم → الشرح المطوي** ([`js/dashboard.js`](js/dashboard.js) + [`css/styles.css`](css/styles.css)):

1. **شريط حالة علوي** (`_statusBar`): الأسبوع X/12 · اليوم (UPPER A / راحة) · الحالة (جاهز/جلسة نشطة/راحة) · شارة Deload في أسابيع ٤/٨/١٢.
2. **Hero مختصر** (`_heroConcise`): «برنامج تضخيم ١٢ أسبوع مصمم لجيمك» + وصف سطر واحد + زرّا «ابدأ تمرين اليوم» و«عرض خطة الأسبوع».
3. **بطاقة تمرين اليوم**: كما في 9.14.10/11 (اسم · تركيز · ٤ إحصاءات: مدة/تمارين/سيتات/راحة · أزرار).
4. **جدول الأسبوع**: شريط الأيام السبعة بحالاتها (موجود — أُعيد ترتيبه أعلى).
5. **قسم «التالي عليك»** (`_nextUpCard`): بلا جلسة → «▶ ابدأ تمرين {اليوم} الآن» (أو رسالة راحة)؛ مع جلسة نشطة → «التالي في جلستك: {التمرين} · {الوزن} · {التكرارات}» (يتخطّى ما سُجِّل) + «ارجع للجلسة». يحوّل الصفحة من عرض إلى مساعد تمرين.

تحقّق آلي بـ headless Chrome (خامل + جلسة نشطة فعلية): الترتيب صحيح، شريط الحالة دقيق، «التالي عليك» يعرض التمرين التالي ويتخطّى المُسجَّل، صفر أخطاء. bump إلى 9.14.12.
