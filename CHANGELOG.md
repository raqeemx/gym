...
## [V9.14.23 — ترقيم السيتات يستثني التسخين (آخر سيت 24 لا 26)] — 2026-06-04

**العَرَض:** يوم UPPER A (24 سيت) كان آخر سيت فيه يحمل رقم **«26»** بينما الشريط السفلي يعرض /24. (ظهر بعد 9.14.22 لأن إصلاح الاقتطاع كشف آخر سيت كان مخفياً).

**السبب الجذري:** عدّاد ترقيم الخطوات `counter.n` كان يزيد لسيتات **التسخين** أيضاً (٢ تسخين + ٢٤ سيت = ٢٦)، فترقيم السيتات العاملة يبدأ بعد التسخين وينتهي عند 26.

**الإصلاح** ([`js/program-render.js`](js/program-render.js)): سيتات التسخين لم تعد تستهلك ترقيم السيتات — تحمل شارة 🔥 بدل رقم. الآن ترقيم السيتات العاملة 1→N يطابق عدّ اليوم والشريط السفلي تماماً.

تحقّق آلي بـ headless Chrome لكل الأيام: رقم آخر سيت = عدّ اليوم = عدد الخطوات (UPPER A=24, ARMS A=20, LEGS=15, ARMS B=26…)، وسيتات التسخين 🔥. bump إلى 9.14.23.

---

## [V9.14.22 — إصلاح اقتطاع آخر تمرين («Reverse Fly لا تظهر بالكامل»)] — 2026-06-03

**السبب الجذري:** جسم اليوم `.dy.open .dby` كان سقفه `max-height:14000px` مع `overflow:hidden`، بينما ارتفاع محتوى يوم BACK & WINGS الكامل **16584px** (السيتات صارت أطول مع خانات الإدخال + البدائل على كل سيت). فالـ~١٦٠٠px الأخيرة — زوج Upper Back ↔ Reverse Fly — كانت **تُقتطع**، فلا يظهر آخر تمرين.

**الإصلاح** ([`css/styles.css`](css/styles.css)): رفع السقف إلى `30000px` (هامش وافٍ يغطّي أطول يوم وكل مسارات الفتح). تحقّق آلي بـ headless Chrome: المحتوى الكامل ١٦٥٨٤px يُعرض بلا اقتطاع، وآخر سيت Reverse Fly ضمن المنطقة المرئية.

---

## [V9.14.21 — إصلاح عدّ السيتات + زر «مشغول؟» على كل السيتات] — 2026-06-03

**عدّ السيتات (٢٤ تظهر ٢٠/٢٣):** السبب أن السيت الأخير (`last:true` «✓ انتهيت») كان يُعلَّم `.done`، وكل الاستعلامات تستثني `.done` — فلا يُحسب **ولا يُحقن له إدخال (غير قابل للتسجيل)**؛ كما أن عدّ بطاقة اليوم كان رقماً يدوياً (`stats.sets`) قد لا يطابق الخطوات. الإصلاح:
- `last:true` صار صنفه `.last-set` بدل `.done` ([`js/program-render.js`](js/program-render.js)) → السيت الأخير سيت حقيقي قابل للتسجيل ويُحسب.
- عدّ بطاقة اليوم والشريط السفلي والرئيسية يُحسب **ديناميكياً** من الخطوات الفعلية (يستثني الراحة/التسخين) فيتطابق الجميع.

**زر «مشغول؟» لا يظهر على كل السيتات:** كان يُحقن مرة واحدة لكل تمرين فقط، ويُتخطّى للتمارين بلا بدائل في الكتالوج. الإصلاح ([`js/substitutions.js`](js/substitutions.js)): يُحقن على **كل سيت** قابل للتتبّع؛ والتمارين بلا بدائل جاهزة تفتح مباشرةً **«اختر من كل الأجهزة المتاحة»**.

تحقّق آلي بـ headless Chrome: بطاقة اليوم ٢٤ = الشريط السفلي ٢٤، السيت الأخير له خانة إدخال، زر «مشغول؟» على الـ٢٤ سيتاً (وتمرين بلا بدائل يفتح كل الأجهزة)، صفر أخطاء. bump إلى 9.14.21.
